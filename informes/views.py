...
def datospdf(socio_id, mes, anio):
    ...
    hide_consumos_totales = any(
        columna2 == 0 or columna2 is None for _, columna2 in consumos_totales)
...
def nuevo_enviar_correo(request, socio_id):
    ...
    if enviarGastoEmail(socio_id, resultados, informe_data):
        response = {'status': 'success',
                    'message': 'Promoción enviada a todos los socios'}
    else:
...
def enviar_correo_todoss(request):
    ...
    if enviar_correo_todos():
        response = {'status': 'success',
                    'message': 'Correo enviado a todos los socios'}
    else:
        response = {'status': 'error', 'message': 'Ups. algo salío mal'}
