...
def principal_resumen(request):
    ...
    for month in range(1, 13):
        ...
        total_normales = sum(float(descuento.consumo_total)
                             if descuento.consumo_total is not None else 0 for descuento in descuentos_normales)

        # Sumar los valores de consumo en descuentos por cuotas
        total_cuotas = sum(float(descuento.consumo_total)
                           if descuento.consumo_total is not None else 0 for descuento in descuentos_cuotas)
    ...
    total_comision = sum(
        float(suma.comision) if suma.comision is not None else 0 for suma in comision)
    ...
    if periodoid:
        ...
        if periodo_seleccionado:
            ...
            rendered_table = render_to_string(
                "lista_pagos_partial.html", context)
...
def listar_pagos_pendientes(request):
    ...
    if periodoid:
        ...
        if periodo_seleccionado:
            ...
            print(periodo_seleccionado)
            pagos = pagos_pendientes.objects.filter(fecha_consumo__range=(
                fechas_periodo['fecha_inicio'], fechas_periodo['fecha_fin']))
            context = {
                'pagos': pagos,
                'periodos': periodos,
                'anos': anos,
    ...
    if periodoid:
        ...
        if periodo_seleccionado:
            ...
            pagos = Pagos_cuotas.objects.filter(
                ...
                Q(fecha_descuento__lt=fechas_periodo['fecha_inicio'],
                  estado="False")
            ).order_by("estado")
            ...
            rendered_table = render_to_string(
                "pagos_cuotas_partial.html", context)
            return JsonResponse({"rendered_table": rendered_table})
    ...
    if request.method == 'GET':
        ...
        if periodo_seleccionado and periodo_seleccionado.fecha_inicio <= datetime.strptime(fecha, '%Y-%m-%d').date() <= periodo_seleccionado.fecha_fin:
            if (float(cantidad) <= cupo.cupo):
                ...
                messages.warning(
                    request, 'La cantidad que desea ingresar es superior a la brindada por el proveedor')
            return redirect('/listar_pagos/')
        else:
            messages.warning(
                request, f'La fecha que ingresaste no esta dentro del periodo actual de {periodo_seleccionado.nombre}')
            users = User.objects.filter(is_active=True)
            socios = Socios.objects.filter(user__in=users)
            proveedores = Proveedor.objects.filter(estado=True)
            return render(request, 'agregar_pago.html', {'socios': socios, 'proveedores': proveedores})
    ...
    if request.method == 'GET':
        ...
        if periodo_seleccionado and periodo_seleccionado.fecha_inicio <= datetime.strptime(fecha, '%Y-%m-%d').date() <= periodo_seleccionado.fecha_fin:
            if (float(cantidad) <= 10000):
                pagos = pagos_pendientes.objects.create(
                    socio=socios, consumo_total=cantidad, fecha_consumo=fecha, descripcion=descripcion)
                messages.success(request, 'Exito')
        ...
        with transaction.atomic():
            if request.method == 'GET':
                ...
                if fecha_actual.day < 15:
                    ...
                    next_month = fecha_actual.replace(
                        day=1) + timedelta(days=32)
                    fecha_inicio_pagos = next_month.replace(day=15)
                cantidadval = Decimal(cantidad)
                n_cuota = Decimal(numero_cuota)
                valor_cuo = cantidadval / n_cuota
                valor_cuo = valor_cuo.quantize(Decimal('0.00'), rounding=ROUND_DOWN)

                sobrante = cantidadval - (valor_cuo * n_cuota)

                print(sobrante, cantidadval, n_cuota)
                ...
                if periodo_seleccionado and periodo_seleccionado.fecha_inicio <= datetime.strptime(fecha_descuento, '%Y-%m-%d').date() <= periodo_seleccionado.fecha_fin:
                    ...
                    fechas_pago = [
                        fecha_inicio_pagos + timedelta(days=(30 * i)) for i in range(int(numero_cuota))]
                    
                    valor_cuota1 = Decimal('0.00')

                    for numero_cuota, fecha_pago in enumerate(fechas_pago, start=1):
                        if numero_cuota == len(fechas_pago):
                            valor_cuota1 = valor_cuo + sobrante
                        else:
                            valor_cuota1 = valor_cuo
                        detalle_cuotas = Detalle_cuotas(
                            pago_cuota=pagoscuotas,
                            numero_cuota=numero_cuota,
                            fecha_descuento=fecha_pago,
                            valor_cuota=valor_cuota1,
                            socio=socios, 
                            proveedor=proveedores
                        )
                        # detalle_cuotas.valor_cuota = valor_cuota1  
                        detalle_cuotas.save()
                        print(valor_cuota1, sobrante)

                    messages.success(
                        request, 'Se ha agregado con éxito el descuento por cuotas')
                    return redirect('/lista_pagos_cuotas/')
                else:
                    messages.warning(
                        request, f'La fecha que ingresaste no está dentro del período actual de {periodo_seleccionado.nombre}')
                    users = User.objects.filter(is_active=True)
                    socios = Socios.objects.filter(user__in=users)
                    proveedores = Proveedor.objects.filter(estado=True)
                    return render(request, 'agregar_pago_cuotas.html', {'socios': socios, 'proveedores': proveedores})
...
def editar_pago_pendiente(request, pago_id):
    if request.method == 'GET':
        ...
        pagos.fecha_consumo = request.POST.get('fecha_consumo')
        pagos.estado = request.POST.get('estado')
        pagos.descripcion = request.POST.get('descripcion', '')
        pagos.save()
...
def detalles_cuota(request, pago_cuota_id):
    detalle_cuotas = Detalle_cuotas.objects.filter(
        pago_cuota=pago_cuota_id).order_by("estado", "numero_cuota")
...
def detalles_cuota_pendiente(request, pago_cuota_id):
    detalle_cuotas = detalle_cuotas_pendientes.objects.filter(
        pago_cuota_pendientes=pago_cuota_id).order_by("estado", "numero_cuota")
    ...
    try:
        with transaction.atomic():
            ...
            if cuota_actual.numero_cuota == 1:
                ...
                pago_cuotas = Pagos_cuotas.objects.get(
                    id=detalle_cuotas.pago_cuota.id)
                ...
                if cuota_anterior is not None and cuota_anterior.estado:
                    ...
                    messages.warning(
                        request, 'No se puede pagar la cuota actual. La cuota del mes anterior no ha sido pagada.')
                    return redirect('/detalles_cuota/' + str(detalle_cuotas.pago_cuota.id))
    except Exception as e:
...
def eliminar_pago_cuota(request, det_cuo_id):
    try:
        with transaction.atomic():
            ...
            cuotas_canceladas = Detalle_cuotas.objects.filter(
                pago_cuota=pago, estado=True)

            if cuotas_canceladas.exists():
                messages.warning(
                    request, 'No se puede eliminar el descuento. Hay cuotas canceladas asociadas a este pago.')
                return redirect('/lista_pagos_cuotas')
...
def eliminar_pago_cuota_pendiente(request, det_cuo_id):
    try:
        with transaction.atomic():
            ...
            cuotas_canceladas = detalle_cuotas_pendientes.objects.filter(
                pago_cuota_pendientes=pago, estado=True)
            ...
            detalle_cuotas_pendientes.objects.filter(
                pago_cuota_pendientes=pagos_cuo).delete()
            pagos_cuo.delete()
            messages.success(
                request, 'Se ha eliminado el pago de la cuota exitosamente.')
            return redirect('/lista_pagos_cuotas_pendientes/')
...
def verificar_registros(request):
    if request.method == 'POST':
        ...
        for i in range(cant_registros):
            ...
            datos = {}
            for campo in campos:
                # Obtenemos el valor del campo para el registro actual
                datos[campo] = registros_formulario.getlist(campo)[i]
            # Agregamos la nueva columna 'estilo_color' con valor 'normal'
        ...
        if cant_errores > 0:
            response = {
                ...
                # Agrega los registros con estilo a la respuesta
                'registros': registros_con_estilo
            }
        else:
            response = {
                'status': True,
...
def generar_reporte_pdf(request):
    ...
    response[
        'Content-Disposition'] = f'attachment; filename="reporte_consumo_proveedores_{mes}_{anio}.pdf"'

    buffer = BytesIO()

    pisa.showLogging()
    pdf = pisa.pisaDocument(BytesIO(html.encode("UTF-8")),
                            buffer, pagesize=landscape((14*inch, 8.5*inch)))
    ...
    try:
        if request.method == 'GET':
            ...
            if periodo_seleccionado and periodo_seleccionado.fecha_inicio <= datetime.strptime(fecha_descuento, '%Y-%m-%d').date() <= periodo_seleccionado.fecha_fin:
                ...
                messages.success(
                    request, 'Se ha agregado con exito el descuento por cuotas')
                return redirect('/lista_pagos_cuotas/')
            else:
                messages.warning(
                    request, f'La fecha que ingresaste no está dentro del período actual de {periodo_seleccionado.nombre}')
        ...
        print("Error:", e)
        return HttpResponseRedirect('/lista_pagos_cuotas/')
...
def buscar_pagos_pendientes(request):
    if request.method == "POST":
        ...
        if criterio:
            # Realiza las consultas según el criterio y el valor ingresados
            if criterio == 'nombres':
                pagos = pagos_pendientes.objects.filter(socio__user__first_name__icontains=valor) | pagos_pendientes.objects.filter(
                    socio__user__last_name__icontains=valor)
            elif criterio == 'proveedor':
                pagos = pagos_pendientes.objects.filter(
                    proveedor__nombre__icontains=valor)
            elif criterio == 'fecha':
                pagos = pagos_pendientes.objects.filter(
                    fecha_consumo__icontains=valor)
        ...
        # Renderiza una tabla parcial con los resultados
        rendered_table = render_to_string("list_pag_pen_par.html", {
                                          "pagos": pagos_paginados})
...
def lista_pagos_cuotas_pendientes(request):
    ...
    if periodoid:
        ...
        if periodo_seleccionado:
            ...
            rendered_table = render_to_string(
                "list_pg_cuo_pen_par.html", context)
            return JsonResponse({"rendered_table": rendered_table})
...
def registra_pago_cuota_pen(request, det_cuo_id):
    try:
        with transaction.atomic():
            detalle_cuotas = detalle_cuotas_pendientes.objects.get(
                id=det_cuo_id)
            cuota_actual = detalle_cuotas.numero_cuota

            if cuota_actual == 1:
                detalle_cuotas.estado = True
                pago_cuotas = pagos_cuotas_pendientes.objects.get(
                    id=detalle_cuotas.pago_cuota_pendientes.id)
                pago_cuotas.cuota_actual += 1
...
def actualizar_descripcion_pendientes(request, id_pago_cu):
    if request.method == 'POST':
        descri = request.POST.get('descripcion', '')
        try:
            pago_cu = pagos_cuotas_pendientes.objects.get(id=id_pago_cu)
            pago_cu.descripcion = descri
            pago_cu.save()
            response_data = {'success': True, 'message': 'Descripción actualizada correctamente.'}
        except Pagos_cuotas.DoesNotExist:
